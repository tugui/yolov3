...
from utils.lsam import LSAM
from utils.lsam2 import LSAM2
...
def create_modules(module_defs, img_size, cfg):
    ...
    img_size = [img_size] * 2 if isinstance(
        img_size, int) else img_size  # expand if necessary
    ...
    for i, mdef in enumerate(module_defs):
        ...
        if mdef['type'] == 'convolutional':
            ...
            k = mdef['size']  # kernel size
            stride = mdef['stride']
            maxpool = nn.MaxPool2d(kernel_size=k,
                                   stride=stride,
                                   padding=(k - 1) // 2)
            ...
            if ONNX_EXPORT:  # explicitly state size, avoid scale_factor
                ...
                modules = nn.Upsample(size=tuple(
                    int(x * g) for x in img_size))  # img_size = (320, 192)
            else:
            ...
            modules = FeatureConcat(layers=layers)
                ...
                'type'] == 'multiply':  # nn.Sequential() placeholder for 'multiply' layer
            layers = mdef['from']
            filters = output_filters[layers[0] + 1 if layers[0] > 0 else
                                     layers[0]]  # number of output channels
            routs.extend([i + l if l < 0 else l for l in layers])
            modules = FeatureMultiply(layers=layers)

        elif mdef[
                'type'] == 'multiply2':  # nn.Sequential() placeholder for 'multiply2' layer
            layers = mdef['from']
            filters = output_filters[layers[0] + 1 if layers[0] > 0 else
                                     layers[0]]  # number of output channels
            routs.extend([i + l if l < 0 else l for l in layers])
            modules = FeatureMultiply2(layers=layers)

        elif mdef[
                'type'] == 'bam':  # nn.Sequential() placeholder for 'bam' layer
            modules = BAM(output_filters[-1])

        elif mdef[
                'type'] == 'lsam':  # nn.Sequential() placeholder for 'lsam' layer
            modules = LSAM(output_filters[-1], mdef['size'], mdef['block_num'], output_filters[-1])

        elif mdef[
                'type'] == 'lsam2':  # nn.Sequential() placeholder for 'lsam2' layer
            modules = LSAM2(output_filters[-1], mdef['size'], mdef['block_num'], output_filters[-1])

        elif mdef['type'] == 'channelpool':
            modules = ChannelPool()
            ...
            try:
                ...
                bias[:, 4] += -4.5  # obj
                bias[:, 5:] += math.log(
                    0.6 / (modules.nc - 0.99))  # cls (sigmoid(p) = 1/nc)
...
class YOLOLayer(nn.Module):
    def __init__(self, anchors, nc, img_size, yolo_index, layers, stride):
        ...
        if ONNX_EXPORT:
            ...
            self.create_grids(
                (img_size[1] // stride,
                 img_size[0] // stride))  # number x, y grid points
...
class Darknet(nn.Module):
    ...
    def __init__(self, cfg, img_size=(416, 416), verbose=False):
        ...
        self.info(
            verbose) if not ONNX_EXPORT else None  # print model description
        ...
        if not augment:
            ...
            for i, xi in enumerate((
                    ...
                    torch_utils.scale_img(
                        x.flip(3), s[0],
                        same_shape=False),  # flip-lr and scale
    ...
    def forward_once(self, x, augment=False, verbose=False):
        ...
        for i, module in enumerate(self.module_list):
            ...
            if name in [
                    'WeightedFeatureFusion', 'FeatureConcat',
                    'FeatureMultiply', 'FeatureMultiply2'
            ]:  # sum, concat, multiply
                if verbose:
                ...
                x = module(
                    x, out
                )  # WeightedFeatureFusion(), FeatureConcat(), FeatureMultiply()
            elif name == 'YOLOLayer':
                yolo_out.append(module(x, out))
            else:  # run module directly, i.e. mtype = 'convolutional', 'upsample', 'maxpool', 'batchnorm2d', 'channelpool', 'bam' etc.
                x = module(x)
    ...
    def fuse(self):
        ...
        self.info(
        ) if not ONNX_EXPORT else None  # yolov3-spp reduced from 225 to 152 layers
...
def get_yolo_layers(model):
    return [
        i for i, m in enumerate(model.module_list)
